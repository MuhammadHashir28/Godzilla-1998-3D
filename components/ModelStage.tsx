  ...
  return (
    <Html center>
      <div className="monster-loader">
        <div className="loader-circle"><span /></div>
        <div className="loader-title">AWAKENING...</div>
        <div className="loader-line">
          <div className="loader-progress" style={{ width: `${progress}%` }} />
        </div>
        <div className="loader-number">{Math.round(progress)}%</div>
      </div>
    </Html>
  );
}
...
function Model({
  ...
  return (
    <group ref={ref} rotation={[0, config.rotationY, 0]} position={config.position} scale={config.scale}>
...
export default React.memo(function ModelStage({
  ...
  return (
    <Canvas
      ...
      dpr={[1, 1]}                    // ✅ Fixed DPR — no upscaling until loaded
      frameloop="always"              // ✅ Fixed — "demand" breaks useFrame animations
      camera={{ position: finalCameraPosition, fov }}
      gl={{
        antialias: true,
        alpha: true,
...
/* ---------------- PRELOAD only the model in use ---------------- */
// ✅ Call this from the page that uses the stage, not here globally:
// useGLTF.preload(modelPaths.a) or useGLTF.preload(modelPaths.b)